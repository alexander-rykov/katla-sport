...
export class ProductFormComponent implements OnInit {
  ...
  product = new Product(0, "PROD1", "New Product", 0, "New Product Description", "MANUFACT", 10, false, "");
  existed = false;
  categoryId: number;
  ...
  onDelete() {
    this.productService.setProductStatus(this.product.id, true).subscribe(c => this.product.isDeleted = true);
  }

  onUndelete() {
    this.productService.setProductStatus(this.product.id, false).subscribe(c => this.product.isDeleted = false);
  }
